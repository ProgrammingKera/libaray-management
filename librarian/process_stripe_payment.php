    ...
    if ($paymentIntent->status == 'succeeded') {
        ...
        try {
            ...
            $stmt = $conn->prepare("
                ...
                VALUES (?, ?, ?, 'stripe', ?)
            ");
            $stmt->bind_param("idds", $fineId, $fine['user_id'], $paymentAmount, $receiptNumber);
            $stmt->execute();
            ...
            $notificationMsg = "Your payment of $" . number_format($paymentAmount, 2) . " for the fine related to '{$fine['book_title']}' has been processed successfully via Stripe.";
